...
export default function ModalVettori({ open, setOpen, getRow, datiForm, setDatiForm, titleModal}) {
    ...
    const handleOnSubmit = async (dati) => {
        ...
        const { data } = await clientAxios.post('vettori', dati)
        console.log(data)
        ...
        if( dati.uuid_utente == '' ){
            ...
            if(data.id_utente){
                ...
                setDatiAlert({
                    title: "Vettore inserito correttamente",
                    open: true,
                });
                getRow();
                setOpen(false);
            ...
            const { data } = await clienteTokenAxios.put(`vettori/${dati.uuid_utente}`, dati)
            console.log(data)
    
            if(data.ok){
                setDatiForm({})
                setDatiAlert({
                    title: "Vettore modificato correttamente",
                    open: true,
                });
                getRow();
                setOpen(false);
