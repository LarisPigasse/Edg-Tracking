...
function Sidebar() {
    ...
    return (
        <div className={`p-2 pt-4 pl-3 bg-neutral-200 h-screen ${open ? "w-64" : "w-16"} relative duration-300`}>
            <BsArrowLeftShort 
            className={`bg-white text-neutral-900 text-2xl rounded-full absolute -right-3 top-6 
                        border border-bg-neutral-700 cursor-pointer ${!open && "rotate-180"}`}
            onClick={() => setOpen(!open)}
            />
            <div className='inline-flex'>
                <img src="https://tools.expressdeliverygroup.com/assets/img/iniziale-white.png" className=' w-10 p-2 bg-cyan-900 rounded-full cursor-pointer block mr-8' />
            ...
            <div class="absolute bottom-0 left-0 px-1 w-full border border-t-neutral-300 text-2xl block float-left text-gray-600">
               <div className='flex'>
                    <div className='hover:text-cyan-600 hover:bg-gray-100 p-2 rounded cursor-pointer m-1'><HiOutlineCog/></div>
                </div> 
            </div>
        </div>
